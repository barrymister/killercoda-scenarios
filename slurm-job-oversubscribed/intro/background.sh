...
# FAULT: a batch script requesting far more CPUs than the node has -> PENDING forever
cat >/root/train.sh <<'EOS'
#!/bin/bash
#SBATCH --job-name=train
#SBATCH --output=/root/train.out
#SBATCH -c 64
echo "training done on $(hostname)"
EOS
chmod +x /root/train.sh
sbatch /root/train.sh >/dev/null 2>&1 || true
sleep 2
