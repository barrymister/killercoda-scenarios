...
# Submit a long-running job so the learner has something to manage
sbatch --job-name=longsleep --output=/root/sleep.out --wrap='sleep 3600' >/dev/null 2>&1 ||   (echo -e '#!/bin/bash\n#SBATCH --job-name=longsleep\n#SBATCH --output=/root/sleep.out\nsleep 3600' >/root/sleep.sh && sbatch /root/sleep.sh >/dev/null 2>&1)
sleep 2
